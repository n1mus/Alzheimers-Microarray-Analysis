...
//  ccdr2
//
//  Created by Bryon Aragam (local) on 12/19/16.
...
//
// Stores a matrix in sparse format, originally intended as a memory-efficient
//   data structure for updating and accessing the entries of the adjacency
//   matrix of a directed acyclic graph (DAG). This is a generic data structure
//   for any sparse matrix.
//
//   The SparseMatrix format consists of two essential components that store the
//     standard column-major implementation of a sparse matrix with a pair of
//     (row, val) vectors for EACH column 1,...,p.
//
//     1) rows: This is a vector of vectors, where the jth vector stores the
//                  indices of the nonzero ROWS in column j; so that
//                  rows[j][k] = i represents the edge a_ij.
//     2) vals: Also a vector of vectors, where the jth vector stores the VALUES
//                  of the nonzero entries in column j; so that
//                  vals[j][k] = val represents the value of edge a_ij.
//
...
//        k      = SPARSE row index
//
//       Finally, note that columns are always accessed "as-is", so there should be
//       no confusion regarding column indices.
//

//
// nonzero
//
//   Returns true if the absolute value of z is larger than the threshold et for
//   nonzero values (see ZERO_THRESH)
//
...
public:
    ...
    // Constructors
    //
    SparseMatrix(int sizeOfMatrix);                                  // Default Constructor

    SparseMatrix(const std::vector< std::vector<int> >& rows_in,     //
    ...
    // Mutator functions
    //
    void setValueBySparseIndex(int j, int k, double v);                             // set the value of an _existing_ edge in the model
    void setValue(int row, int col, double v);                                      // set the value of an _existing_ edge in the model
    double addEdge(int row, int col, double val);                                   // add a new edge and return the difference 
    double updateEdge(int j, int k, double val);                                    // update the value of an edge and return the difference 
    double update(int row, int col, double val);                                    // update the value of an edge and return the difference 
    ...
    SparseMatrix(Rcpp::List sbm);          // Explicit Constructor
...
//
// Initialization method
//   Separate method for initializing main values of a SparseMatrix object; used since initializer lists
//   are only allowed in C++11, which R does not support on CRAN yet.
//
void SparseMatrix::init(const std::vector< std::vector<int> >& rows_in,
                             const std::vector< std::vector<double> >& vals_in,
                             const std::vector< std::vector<int> >& blocks_in,
                             const std::vector<double>& sigmas_in){
...
//
SparseMatrix::SparseMatrix(int sizeOfMatrix){
...
//
SparseMatrix::SparseMatrix(const std::vector< std::vector<int> >& rows_in,
                                     const std::vector< std::vector<double> >& vals_in,
                                     const std::vector< std::vector<int> >& blocks_in){
...
// j = (true) column index
// k = sparse row index
int SparseMatrix::row(int j, int k) const{
    return rows[j][k];
}

double SparseMatrix::value(int j, int k) const{
    return vals[j][k];
}

int SparseMatrix::block(int j, int k) const{
    return blocks[j][k];
}

double SparseMatrix::sigma(int j) const{
    return sigmas[j];
}
...
//
int SparseMatrix::find(int row, int col) const{
...
//
double SparseMatrix::findValue(int row, int col) const{
    //
...
double SparseMatrix::getSiblingValue(int j, int k) const{
    //
...
bool SparseMatrix::isEmpty(int j) const{
    if(rowsizes(j) > 0)
        return false;
    else
        return true;
...
int SparseMatrix::activeSetSize() const{
...
//
int SparseMatrix::recomputeActiveSetSize(bool reset){
...
void SparseMatrix::setValueBySparseIndex(int j, int k, double v){

    #ifdef _DEBUG_ON_
        // in debug mode, check for existence of edge first
        if(k >= rows[j].size()){
            FILE_LOG(logERROR) << "Warning: setValueBySparseIndex called on edge that does not exist in model!" << std::endl;
...
void SparseMatrix::setValue(int row, int col, double v){
    //
    // Need to check validity of input: rows, cols < pp
    //

    vals[col][find(row, col)] = v;
}

double SparseMatrix::addEdge(int row, int col, double val){
    rows[col].push_back(row); // add edge (row, col)
    vals[col].push_back(val); // add value
    ...
    //        (e.g. L1, L2, etc)
    double err = val;   // Since we are adding a new block, the old coefficient values must be zero,
                        //  so err = val - 0 = val

    return err;
}

double SparseMatrix::updateEdge(int j, int k, double val){
    ...
    double oldval = vals[j][k];

    #ifdef _DEBUG_ON_
        FILE_LOG(logDEBUG2) << "Updating block at (" << rows[j][k] << ", " << j << "):  " << oldval << " --> " << val;
    #endif

    // Update the values
    setValueBySparseIndex(j, k, val);

    // NOTE: These values may be negative; it is up to the getError() function to implement the desired error function
    //        (e.g. L1, L2, etc)
    double err = val - oldval;

    return err;
}

double SparseMatrix::update(int row, int col, double val){
    int found = find(row, col); // potential bottleneck in the code!
    double err = 0;

    if(found >= 0){ // if the edge exists in the sparse matrix, update it's value
        double oldval = value(col, found);
        setValueBySparseIndex(col, found, val);
        err = val - oldval;
    } else{ // if the edge doesn't exist, add it
        err = addEdge(row, col, val);
    }
...
void SparseMatrix::setSigma(int j, double s){
    sigmas[j] = s;
}
...
std::vector<double> SparseMatrix::addBlock(int row, int col, double valij, double valji){
...
//       j = COLUMN index
//       k = sparse ROW index
std::vector<double> SparseMatrix::updateBlock(int j, int k, double valij, double valji){
    ...
    // Update the values
    setValueBySparseIndex(j, k, valij);
    setValueBySparseIndex(rows[j][k], blocks[j][k], valji);
...
void SparseMatrix::print(int r) const{
    ...
    void SparseMatrix::init(Rcpp::List rows_in,
                                 Rcpp::List vals_in,
                                 Rcpp::List blocks_in,
                                 Rcpp::NumericVector sigmas_in){
    ...
    // Takes in an R list containing the components of an R SparseMatrix object:
    //   list(rows, vals, blocks, sigmas)
    SparseMatrix::SparseMatrix(Rcpp::List sbm){
